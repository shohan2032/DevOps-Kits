...
async function runCommand(command) {
  try {
    const {stdout, stderr} = await execAsync(command); // Replace with your command
    if (stderr) {
      console.error(`Error in ${command}: ${stderr}`);
    }
    console.log(`Output for ${command}: ${stdout}`);
  } catch (error) {
    console.error(`Execution failed ${command}: ${error.message}`);
  }
}
...
function findBackendPort(subdomain) {
  ...
  console.log(
    `Using default backend port ${DEFAULT_BACKEND_PORT} for subdomain ${subdomain}`
  );
...
// Assign the frontend port
function assignFrontendPort(subdomain) {
  console.log(`Assigning frontend port for subdomain: ${subdomain}`);
  const existingPort = findFrontendPort(subdomain);
  if (existingPort) {
    console.log(
  ...
  let newPort = getNextAvailablePort(FRONTEND_MAP_FILE);
  ...
  console.log(
    `Assigning new frontend port ${newPort} to subdomain ${subdomain}`
  );
  fs.appendFileSync(FRONTEND_MAP_FILE, `${newPort} ${subdomain}\n`, 'utf8');

  return newPort;
}

// Create nginx config
function createNginxConfig(subdomain = 'main', fPort = 3000) {
  console.log(
    `Creating NGINX config for subdomain: ${subdomain} on port: ${fPort}`
  );
    ...
    function callback(err, stdout, stderr) {
      if (err) {
        if (stderr.includes('File exists')) {
          console.warn(
            `Symbolic link ${sitesEnabledLocation + fileName} already exists. Skipping creation.`
          );
        } else {
...
function slugify(text = '', replacement = '-') {
  console.log(`Slugifying text: ${text} with replacement: ${replacement}`);
  if (!text) return '';
  return text
...
function preparePm2Config(name, path, port = 3000) {
  const pm2Config = {
    apps: [
      {
        name,
...
async function main() {
  ...
  const assignedPort = assignFrontendPort(subdomain);

  console.log(
    `Assigned frontend port: ${assignedPort} to subdomain: ${subdomain}`
  );

  const releasePath = `/home/ubuntu/frontend/releases/${subdomain}`;
  const pm2Name = `frontend-${subdomain}`;

  preparePm2Config(pm2Name, releasePath, assignedPort);

  createNginxConfig(subdomain, assignedPort);

  console.log('Script completed successfully.');

  return {
    frontendPort: assignedPort,
    backendPort: findBackendPort(subdomain),
    subdomain: subdomain,
  };
}
